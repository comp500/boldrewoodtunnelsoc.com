...
window.onload=init;
	...
	function init()
	{
		...
		if (player2) {
		    game_loop = setInterval(function() {paint(snake, snake2)}, 100);
		}
		else {
		    game_loop = setInterval(function() {paint(snake, snake)}, 100);
		}
		document.getElementById("Retry").innerHTML = '<button style="width:150px;height:80px;font-size:30px;" onclick=player2Toggle()>Player 2</button>';
	}
	...
	function paint(snake, snake2) {
		if (!(snake.check_collision() || snake2.check_collision())) {
			...
			canvas.context.strokeStyle = "black";
			canvas.context.strokeRect(0, 0, canvas.width, canvas.height);
			document.getElementById("Retry").innerHTML = '<button style="width:150px;height:80px;font-size:30px;" onclick="init()">Retry</button>';
